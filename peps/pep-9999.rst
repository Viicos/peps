...
.. productionlist:: inline-typed-dict
    ...
    inline_typed_dict_item: `~python-grammar:stringliteral` ":" `~python-grammar:expression`
    inline_typed_dict_comprehension: `~python-grammar:identifier` ":" `~python-grammar:expression` "for" `~python-grammar:identifier` "in" `~python-grammar:expression`
...
.. [#inner-expr] Technically, the inner expression isn't specified in the Python
   ...
   of syntaxes are allowed inside the enclosing ``<`` and ``>`` characters,
   and some of these syntaxes are described in this PEP. In the future,
   new type expressions can be added.
